...
/*
  このプログラムは、指定された複数のIPアドレスに対し、LAN内デバイスの起動状態を監視するものである。1秒ごとに各デバイスへ並行してpingを送信する。
  実現のため、fork() を使用してIPごとに子プロセスを作成する。子プロセスは execlp() で ping コマンド（1回送信、1秒タイムアウト）を実行する。
  ping の終了ステータスに基づき、各デバイスの状態を1秒ごとに更新・表示し続ける。
*/
